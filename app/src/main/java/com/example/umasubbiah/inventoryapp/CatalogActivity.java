...
public class CatalogActivity extends AppCompatActivity implements
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // Find and set empty view on the ListView, so that it only shows when the list has 0 products.
        View emptyView = findViewById(R.id.empty_view);
        stockListView.setEmptyView(emptyView);

        // Setup an Adapter to create a list product for each row of product data in the Cursor.
        // There is no product data yet (until the loader finishes) so pass in null for the Cursor.
        mCursorAdapter = new StockCursorAdapter(this, null);
        stockListView.setAdapter(mCursorAdapter);

        // Setup the product click listener
        stockListView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            @Override
            public void onItemClick(AdapterView<?> adapterView, View view, int position, long id) {
                // Create new intent to go to {@link EditorActivity}
                Intent intent = new Intent(CatalogActivity.this, InfoActvity.class);
    ...
    public void updateQuantity(long id, String name, int price, int quant, String supName, String supMail, int numSold) {
        Uri mUri = ContentUris.withAppendedId(StockEntry.CONTENT_URI, id);
        int newQuant = (quant - numSold);

        ContentValues values = new ContentValues();
        values.put(StockEntry.COLUMN_STOCK_NAME, name);
        values.put(StockEntry.COLUMN_STOCK_PRICE, price);
        values.put(StockEntry.COLUMN_STOCK_QUANTITY, newQuant);
        values.put(StockEntry.COLUMN_STOCK_SUPPLIER, supName);
        values.put(StockEntry.COLUMN_STOCK_SUPPLIER_EMAIL, supMail);
    ...
    private void deleteAllStock() {
        ...
        Log.v("CatalogActivity", rowsDeleted + " rows deleted from stock database");
    }

    @Override
    public boolean onCreateOptionsMenu(Menu menu) {
        // Inflate the menu options from the res/menu/menu_catalog.xml file.
        // This adds menu products to the app bar.
